...
let carouselNext = document.getElementById("carousel-button-next");
let carouselPrev = document.getElementById("carousel-button-prev");

carouselNext.addEventListener("click", movetoNextSlide)

carouselPrev.addEventListener("click", moveToPrevSlide)
...
function dots(){
    ...
    for(let i = 0; i < dots.length; i++){
        ...
        dots[0].className.replace("first-dot", "")
    }
    dots[slidePosition].className += " active";
}

let timerEl = setInterval(movetoNextSlide, 4000);
let stopEl = document.getElementById("stopSlide");


function startTimer(){
        timerEl = setInterval(movetoNextSlide, 4000);
        stopEl.innerHTML = "Stop Slide";
        console.log(stopEl.innerHTML)
}

function stopTimer(){
    timerEl = clearInterval(timerEl);
    stopEl.innerHTML = "Start Slide"; 
    console.log(stopEl.innerHTML)
}
...
stopEl.addEventListener("click", function(){
    if(  stopEl.innerHTML == "Stop Slide"){
        stopTimer()
    }else{
        startTimer()
    } 
